...
public class MrNightmareEnemyManger : MonoBehaviour {
    ...
    void Spawn ()
    {
		...
		else if (mrNightmareHealth.currentHealth <= healthLvlFour && !usedLvlFour)
		{
			...
			LogManager.logManager.AddEvent(Time.time, "Alert;MrNightmare;CallForHelp");
	...
	IEnumerator CallForHellp(int number, float waitTime)
    {
        ...
        while(i < number)
        {
			...
			LogManager.logManager.AddEvent(Time.time, "Enemy;" + enemyIndex + ";MrNightmareSpawn;AtSpawnPoint;" + spawnPointIndex);
